...
def rc2label_dict(modelXbrl, lang="en", table_uri=None):
    return RC2Label(modelXbrl, lang).generate(table_uri)
    
class RC2Label(object):
    ...
    def generate(self, table_uri = None):
        ...
        for table_uri in table_uris:
            ...
            if tblAxisRelSet:
                ...
                d = self.bodyCells(yTopNode, xNodes, zAspectNodes, d)
        return d
    ...
    def bodyCells(self, yParent, xNodes, zAspectNodes, d):
                                ...
                                if label[1]['concept_id'] == value[0:6]:
                                    for child in label[1]['children']:
                                        if (child['concept_id'][4:])==(value[7:]):
                                            value = child['label']
                        if short_y[0]!="R":
                            data_point_def_short  = str(self.roledefinition) + ","+ str(short_x).lower()
                            data_point_def_long  = str(label_x).lower()
                            d[data_point_def_short] = data_point_def_long
                            # Store index as datapoint
                            data_point_def_short  = str(self.roledefinition) + ","+ index_name.lower()
                            data_point_def_long  = str(short_y).lower()
                            d[data_point_def_short] = data_point_def_long

                        else:
                            data_point_def_short  = str(self.roledefinition) + "," + str(short_y).lower() + ","+ str(short_x).lower()
                            data_point_def_long  = str(label_y).lower() + ", "+ str(label_x).lower()
                            d[data_point_def_short] = data_point_def_long
                            data_point_def_short  = str(self.roledefinition) + ","+ str(short_x).lower()
                            data_point_def_long  = str(label_x).lower()
                            d[data_point_def_short] = data_point_def_long
                            data_point_def_short  = str(self.roledefinition) + ","+ str(short_y).lower()
                            data_point_def_long  = str(label_y).lower()
                            d[data_point_def_short] = data_point_def_long
                        fp.clear()
                d = self.bodyCells(yNode, xNodes, zAspectNodes, d)
        return d
